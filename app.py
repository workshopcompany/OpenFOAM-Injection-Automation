...
# 3. 3D Flow Visualization (Optimized for .vtm & Cloud)
vtk_dir = "VTK"
if os.path.exists(vtk_dir):
    st.subheader("3D Flow Visualization")
    
    # Use recursive search to find .vtm files even in subfolders
    # vtm files are MultiBlock datasets generated by OpenFOAM
    all_vis_files = glob.glob(f"{vtk_dir}/**/*.vtm", recursive=True) + \
                    glob.glob(f"{vtk_dir}/**/*.vtk", recursive=True)
    
    water_files = [f for f in all_vis_files if "alpha.water" in f]
    
    if water_files:
        # Improved sorting for timesteps (e.g., _40.vtm, _42.vtm)
        def get_step(f):
            nums = re.findall(r'_(\d+)', f)
            return int(nums[-1]) if nums else 0
        water_files.sort(key=get_step)
        
        latest_file = water_files[-1]
        st.caption(f"Rendering: `{os.path.basename(latest_file)}`")
        
        try:
            # CRITICAL: Start virtual display for Streamlit Cloud (Prevents 5-min freeze)
            if 'xvfb_ready' not in st.session_state:
                pv.start_xvfb()
                st.session_state['xvfb_ready'] = True
            
            # Read and merge MultiBlock data
            ...
            # Use dynamic key based on file timestamp to force refresh
            stpyvista(plotter, key=f"vtm_render_{os.path.getmtime(latest_file)}")
            
        except Exception as e:
            st.error(f"Visualization Error: {e}")
    else:
        # Fallback: list found files for debugging if alpha.water is missing
        st.warning("No 'alpha.water' files found. Found files: " + str([os.path.basename(f) for f in all_vis_files[:3]]))
else:
    st.info("VTK directory not found. Please sync results.")
